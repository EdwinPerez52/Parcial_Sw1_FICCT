...
describe('cliente autenticado', () => {
  ...
  it('authApi.register adjunta el token de invitación', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
    ...
    const res = await authApi.register('Juan Pérez', 'juan@example.com', 'Password123', 'Password123', 'invitacion-valida');
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[0][1]?.body))).toEqual({
      ...
      invitationToken: 'invitacion-valida',
    });
