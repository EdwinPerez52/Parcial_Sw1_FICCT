...
import App from './App';
import { CurrentUser, ProjectSummary, authApi, diagramApi } from '../shared/api/api';
...
export default function AuthApp() {
  ...
  useEffect(() => {
    void authApi.me().then(setUser).catch(cause => {
      // A failed session restore must not prevent a new login attempt.
      setSessionError(messageOf(cause));
      setUser({ authenticated: false, email: null, fullName: null, verified: false, platformAdmin: false, csrfToken: '' });
    });
  }, []);
  ...
  if (!user.authenticated) return <PublicAuth invitationToken={joinToken} forcedRegistration={path === '/register'} initialError={sessionError} onAuthenticated={setUser} />;
...
function PublicAuth({ invitationToken, forcedRegistration, initialError, onAuthenticated }: { invitationToken: string; forcedRegistration: boolean; initialError: string; onAuthenticated: (user: CurrentUser) => void }) {
  const [mode, setMode] = useState<'login' | 'register' | 'forgot'>(forcedRegistration ? 'register' : 'login');
  const [error, setError] = useState(new URLSearchParams(location.search).get('authError') ?? initialError);
  ...
  const submitLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { onAuthenticated(await authApi.login(String(data.get('email')), String(data.get('password')))); }
  ...
  const submitRegister = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSavedEmail(email);
      setNotice(result.message || 'Revisa tu correo para verificar la cuenta antes de iniciar sesión.');
      setMode('login');
    } catch (cause) { setError(messageOf(cause)); }
  };
    ...
    <section className="auth-card" aria-busy={Boolean(invitationToken) && invitationValid === undefined}>
      ...
      {mode === 'login' && <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '12px' }}>
          {(!invitationToken || invitationValid) && <button className="text-button" type="button" onClick={() => { setMode('register'); setError(''); setNotice(''); }}>
            <UserPlus size={16} style={{ verticalAlign: 'middle', marginRight: '6px' }} />Crear una cuenta
          </button>}
