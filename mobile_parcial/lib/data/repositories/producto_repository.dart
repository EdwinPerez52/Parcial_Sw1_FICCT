...
class ProductoRepository {
  ...
  static const String _entity = 'producto';
  static const Uuid _uuid = Uuid();

  Future<List<Producto>> getAll({int page = 0, int size = 20, String? search}) async {
    if (_sync.isOnline) {
      _sync.syncDown().catchError((_) {});
    }

    final localRows = await _db.getCachedEntities(_entity);
    List<Producto> items = localRows.map((e) => Producto.fromJson(e)).toList();
    ...
    if (items.isEmpty && _sync.isOnline) {
      try {
        ...
        final data = await _client.get('/api/producto', queryParams: query);
        if (data is List) {
          ...
          items = refreshed.map((e) => Producto.fromJson(e)).toList();
        }
      } catch (_) {}
    }
  ...
  Future<Producto> getById(String id) async {
    ...
    if (local != null) {
      return Producto.fromJson(local);
    }
    final data = await _client.get('/api/producto/$id') as Map<String, dynamic>;
    final item = Producto.fromJson(data);
    await _db.saveCachedEntity(_entity, id, data, syncStatus: 'synced');
    return item;
  }

  Future<Producto> create(Producto item) async {
    final id = item.id.isNotEmpty ? item.id : _uuid.v4();
    final json = item.toJson()..['id'] = id;
    final createdItem = Producto.fromJson(json);
  ...
  Future<Producto> update(String id, Producto item) async {
    final cached = await _db.getCachedEntity(_entity, id);
    final baseVersion = (cached?['_version'] as int?) ?? 1;

    final json = item.toJson()..['id'] = id;
    final updatedItem = Producto.fromJson(json);
