...
import { Activity, CheckCircle2, MessageSquare, RotateCcw, Save, Send } from 'lucide-react';
import { ActivityItem, CommentItem, VersionItem, collaborationApi } from './api';
...
export function VersionsPanel({ diagramId, diagram, pendingCount, eventSequence, accept }: { diagramId: string; diagram: DiagramModel; pendingCount: number; eventSequence: number; accept: (value: DiagramModel) => void }) {
  ...
  return <div className="versions-panel"><form className="version-form" onSubmit={createVersion}><input name="label" maxLength={180} placeholder="Nombre del hito" required /><button><Save size={14} /> Guardar hito</button></form>
    ...
    {preview && <div className="modal-backdrop"><section className="proposal-modal"><h2>Restaurar “{preview.label}”</h2><p>Se creará una revisión nueva; no se borrará el historial anterior.</p><div className="snapshot-summary"><span>{preview.snapshot.classes.length} clases</span><span>{preview.snapshot.enumerations.length} enumeraciones</span><span>{preview.snapshot.associations.length} asociaciones</span><span>{preview.snapshot.generalizations.length} herencias</span></div><footer><button className="secondary" onClick={() => setPreview(undefined)}>Cancelar</button><button className="primary" disabled={pendingCount > 0} onClick={() => void restore()}><RotateCcw size={15} /> Restaurar como nueva revisión</button></footer></section></div>}
  </div>;
}
