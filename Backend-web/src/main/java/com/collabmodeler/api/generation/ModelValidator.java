...
public class ModelValidator {
    ...
    public void validate(DiagramDocument diagram, String groupId, String artifactId) {
        ...
        for (var item : diagram.classes()) {
            ...
            if (!isChild) {
                var parentLinks = diagram.associations().stream()
                    .filter(link -> link.targetId().equals(item.id()) && link.sourceCardinality().equals("1")
                        && link.targetCardinality().endsWith("*"))
                    .map(link -> link.sourceId()).distinct().toList();
                boolean intermediate = primaryKeys == 2 && parentLinks.size() == 2 && parentLinks.stream().allMatch(parentId -> {
                    var parent = classMap.get(parentId);
                    if (parent == null) return false;
                    var parentKeys = parent.attributes().stream().filter(DiagramDocument.Attribute::primaryKey).toList();
                    if (parentKeys.size() != 1) return false;
                    var parentKey = parentKeys.getFirst();
                    String expectedName = parent.name() + "_" + parentKey.name();
                    String legacyName = parent.name() + "_id";
                    return item.attributes().stream().anyMatch(attribute -> attribute.primaryKey()
                        && attribute.required()
                        && attribute.type().equals(parentKey.type())
                        && (attribute.name().equalsIgnoreCase(expectedName)
                            || attribute.name().equalsIgnoreCase(legacyName)));
                });
                if (primaryKeys != 1 && !intermediate) {
                    throw new ModelValidationException(item.id(), "INVALID_PRIMARY_KEY", "La clase " + item.name() + " debe tener una clave primaria, o dos si es una tabla intermedia (tiene " + primaryKeys + ")");
                }
            } else {
