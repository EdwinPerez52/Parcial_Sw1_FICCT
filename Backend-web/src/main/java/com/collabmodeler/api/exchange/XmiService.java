...
public class XmiService {
    ...
    public String exportXmi(DiagramDocument diagram) {
        try {
            Map<UUID, IntermediateClass> intermediateClasses = intermediateClasses(diagram);
            Set<UUID> intermediateLinks = new HashSet<>();
            intermediateClasses.values().forEach(value -> {
                intermediateLinks.add(value.firstLink().id());
                intermediateLinks.add(value.secondLink().id());
            });
            ...
            for (DiagramDocument.PackageElement value : diagram.packages()) {
                if (value.parentId() == null) writePackage(xml, value, diagram, intermediateClasses, intermediateLinks);
            }
            for (DiagramDocument.ClassElement value : diagram.classes()) {
                if (!packagedMembers.contains(value.id())) {
                    IntermediateClass intermediate = intermediateClasses.get(value.id());
                    if (intermediate == null) writeClass(xml, value, diagram);
                    else writeAssociationClass(xml, intermediate, diagram);
                }
            }
            ...
            for (DiagramDocument.Association value : diagram.associations()) {
                if (!packagedMembers.contains(value.id()) && !intermediateLinks.contains(value.id())) writeAssociation(xml, value);
            }
            xml.writeEndElement();
            xml.writeEndElement();
    ...
    private void writePackage(XMLStreamWriter xml, DiagramDocument.PackageElement value,
                              DiagramDocument diagram, Map<UUID, IntermediateClass> intermediateClasses,
                              Set<UUID> intermediateLinks) throws Exception {
        xml.writeStartElement("packagedElement");
        ...
        for (DiagramDocument.PackageElement child : diagram.packages()) {
            if (value.id().equals(child.parentId())) writePackage(xml, child, diagram, intermediateClasses, intermediateLinks);
        }
        for (UUID memberId : value.memberIds()) {
            ...
            DiagramDocument.Association association = diagram.associations().stream()
                .filter(item -> item.id().equals(memberId)).findFirst().orElse(null);
            if (classValue != null) {
                IntermediateClass intermediate = intermediateClasses.get(classValue.id());
                if (intermediate == null) writeClass(xml, classValue, diagram);
                else writeAssociationClass(xml, intermediate, diagram);
            }
            else if (enumeration != null) writeEnumeration(xml, enumeration);
            else if (association != null && !intermediateLinks.contains(association.id())) writeAssociation(xml, association);
        }
        xml.writeEndElement();
    }
    ...
    private void writeClass(XMLStreamWriter xml, DiagramDocument.ClassElement value,
        ...
        xml.writeAttribute("name", value.name());
        writeAttributes(xml, value, diagram);
        writeGeneralizations(xml, value, diagram);
        xml.writeEndElement();
    }

    private void writeAttributes(XMLStreamWriter xml, DiagramDocument.ClassElement value,
                                 DiagramDocument diagram) throws Exception {
    ...
    private void writeGeneralizations(XMLStreamWriter xml, DiagramDocument.ClassElement value,
                                      DiagramDocument diagram) throws Exception {
    ...
    private void writeAssociationClass(XMLStreamWriter xml, IntermediateClass intermediate,
                                       DiagramDocument diagram) throws Exception {
        DiagramDocument.ClassElement value = intermediate.value();
        DiagramDocument.Association first = intermediate.firstLink();
        DiagramDocument.Association second = intermediate.secondLink();
        String firstEnd = "end_" + value.id() + "_first";
        String secondEnd = "end_" + value.id() + "_second";
        xml.writeStartElement("packagedElement");
        xml.writeAttribute(XMI, "type", "uml:AssociationClass");
        xml.writeAttribute(XMI, "id", "class_" + value.id());
        xml.writeAttribute("name", value.name());
        xml.writeAttribute("memberEnd", firstEnd + " " + secondEnd);
        writeAttributes(xml, value, diagram);
        writeGeneralizations(xml, value, diagram);
        // In the decomposed model each parent-to-intermediate link stores the
        // multiplicity of the opposite association end.
        writeAssociationEnd(xml, firstEnd, first.sourceId(), second.targetCardinality(), first.sourceRole(), "ASSOCIATION");
        writeAssociationEnd(xml, secondEnd, second.sourceId(), first.targetCardinality(), second.sourceRole(), "ASSOCIATION");
        xml.writeEndElement();
    }
    ...
    private void writeAssociation(XMLStreamWriter xml, DiagramDocument.Association value) throws Exception {
        if ("DEPENDENCY".equals(value.relationType())) {
            xml.writeStartElement("packagedElement");
            xml.writeAttribute(XMI, "type", "uml:Dependency");
            xml.writeAttribute(XMI, "id", "association_" + value.id());
            xml.writeAttribute("client", "class_" + value.sourceId());
            xml.writeAttribute("supplier", "class_" + value.targetId());
            if (value.name() != null && !value.name().isBlank()) xml.writeAttribute("name", value.name());
            writeHandleMetadata(xml, value);
            xml.writeEndElement();
            return;
        }
        ...
        xml.writeAttribute(XMI, "id", "association_" + value.id());
        if (value.name() != null && !value.name().isBlank()) xml.writeAttribute("name", value.name());
        xml.writeAttribute("collab", COLLAB, "relationType", value.relationType());
        writeHandleMetadata(xml, value);
        ...
        writeAssociationEnd(xml, sourceEnd, value.sourceId(), value.sourceCardinality(), value.sourceRole(), value.relationType());
        writeAssociationEnd(xml, targetEnd, value.targetId(), value.targetCardinality(), value.targetRole(), "ASSOCIATION");
        xml.writeEndElement();
    }

    private void writeHandleMetadata(XMLStreamWriter xml, DiagramDocument.Association value) throws Exception {
        if (value.sourceHandle() != null && !value.sourceHandle().isBlank())
            xml.writeAttribute("collab", COLLAB, "sourceHandle", value.sourceHandle());
        if (value.targetHandle() != null && !value.targetHandle().isBlank())
            xml.writeAttribute("collab", COLLAB, "targetHandle", value.targetHandle());
    }

    private void writeAssociationEnd(XMLStreamWriter xml, String id, UUID type, String cardinality,
                                     String role, String relationType) throws Exception {
        xml.writeStartElement("ownedEnd");
        xml.writeAttribute(XMI, "type", "uml:Property");
        xml.writeAttribute(XMI, "id", id);
        xml.writeAttribute("type", "class_" + type);
        if ("AGGREGATION".equals(relationType)) xml.writeAttribute("aggregation", "shared");
        if ("COMPOSITION".equals(relationType)) xml.writeAttribute("aggregation", "composite");
    ...
    public ImportResult importXmi(byte[] bytes, String fallbackName) {
        ...
        try {
            ...
            List<Element> packaged = modelElements(document);
            Map<String, Element> elementsById = elementsById(document);
            ...
            for (Element element : packaged) {
                ...
                if (Set.of("uml:Package", "uml:Class", "uml:AssociationClass", "uml:Enumeration", "uml:Association", "uml:Dependency").contains(type)) {
                    ids.put(externalId, stableId(externalId));
                    if ("uml:Package".equals(type)) packageElements.put(externalId, element);
                }
                if (Set.of("uml:PrimitiveType", "uml:DataType", "uml:Enumeration", "uml:Class", "uml:AssociationClass").contains(type)) {
            ...
            for (Element element : packaged) {
                if (!isClassType(xmiType(element))) continue;
                String externalId = xmiAttribute(element, "id");
                if (!ids.containsKey(externalId)) continue;
            ...
            List<DiagramDocument.Association> associations = readAssociations(document, packaged, elementsById, ids, warnings);
            classes = completeAssociationClassKeys(packaged, elementsById, ids, classes, warnings);
            ...
            for (Element element : packaged) {
                String type = xmiType(element);
            if (!Set.of("uml:Package", "uml:Class", "uml:AssociationClass", "uml:Enumeration", "uml:Association", "uml:Dependency",
    ...
    private ImportResult importEnterpriseArchitectNative(Element root, String fallbackName) {
        ...
        for (Element row : eaRows(root, "t_connector")) {
            ...
            if ("Generalization".equalsIgnoreCase(connectorType)) {
                ...
                || "Dependency".equalsIgnoreCase(connectorType)) {
                associations.add(new DiagramDocument.Association(stableId("ea-association:" + connectorId), source, target,
                    ...
                    nullableIdentifier(eaColumn(row, "DestRole"), warnings, connectorId, "ROLE"), "SOURCE",
                    "Aggregation".equalsIgnoreCase(connectorType) ? "AGGREGATION" :
                        "Dependency".equalsIgnoreCase(connectorType) ? "DEPENDENCY" : "ASSOCIATION", 1));
            } else {
    ...
    private List<DiagramDocument.Attribute> readAttributes(Element owner, Map<String, String> typeNames,
        ...
        for (Element attribute : directChildren(owner, "ownedAttribute")) {
            ...
            String typeReference = reference(attribute, "type");
            ...
            if (normalized == null) {
                normalized = "String";
                if (!rawType.isBlank()) warnings.add(warning("UNSUPPORTED_PROPERTY_TYPE",
    ...
    private List<DiagramDocument.Association> readAssociations(Document document, List<Element> packaged,
        ...
        for (Element association : packaged) {
            if ("uml:Dependency".equals(xmiType(association))) {
                String externalId = xmiAttribute(association, "id");
                List<String> clients = references(association, "client");
                List<String> suppliers = references(association, "supplier");
                UUID source = clients.isEmpty() ? null : ids.get(clients.getFirst());
                UUID target = suppliers.isEmpty() ? null : ids.get(suppliers.getFirst());
                if (source != null && target != null) result.add(new DiagramDocument.Association(ids.get(externalId),
                    source, target, "1", "1", nullableIdentifier(association.getAttribute("name"), warnings,
                    externalId, "DEPENDENCY"), null, null, "SOURCE", "DEPENDENCY",
                    handleMetadata(association, "sourceHandle"), handleMetadata(association, "targetHandle"), 1));
                else warnings.add(warning("EXTERNAL_ASSOCIATION_END", "Se omitió una dependencia con clases externas",
                    externalId, "DEPENDENCY"));
                continue;
            }
            if (!isAssociationType(xmiType(association))) continue;
            String externalId = xmiAttribute(association, "id");
            List<Element> ends = new ArrayList<>();
            for (String memberEnd : references(association, "memberEnd")) {
                Element referenced = elementsById.get(memberEnd);
                if (referenced != null) ends.add(referenced);
            }
            ends.addAll(directChildren(association, "ownedEnd"));
            ...
            UUID source = ids.get(reference(sourceEnd, "type"));
            UUID target = ids.get(reference(targetEnd, "type"));
            if (source == null || target == null) {
            ...
            UUID associationId = "uml:AssociationClass".equals(xmiType(association))
                ? stableId("association:" + externalId) : ids.get(externalId);
            if ("uml:AssociationClass".equals(xmiType(association))) {
                UUID intermediate = ids.get(externalId);
                String sourceMultiplicity = cardinality(sourceEnd, warnings);
                String targetMultiplicity = cardinality(targetEnd, warnings);
                result.add(new DiagramDocument.Association(stableId("association-class-link:" + externalId + ":source"),
                    source, intermediate, "1", targetMultiplicity, null,
                    nullableIdentifier(sourceEnd.getAttribute("name"), warnings, xmiAttribute(sourceEnd, "id"), "ROLE"),
                    null, "TARGET", "ASSOCIATION", null, null, 1));
                result.add(new DiagramDocument.Association(stableId("association-class-link:" + externalId + ":target"),
                    target, intermediate, "1", sourceMultiplicity, null,
                    nullableIdentifier(targetEnd.getAttribute("name"), warnings, targetEndId, "ROLE"),
                    null, "TARGET", "ASSOCIATION", null, null, 1));
                continue;
            }
            result.add(new DiagramDocument.Association(associationId, source, target,
                ...
                nullableIdentifier(sourceEnd.getAttribute("name"), warnings, xmiAttribute(sourceEnd, "id"), "ROLE"),
                nullableIdentifier(targetEnd.getAttribute("name"), warnings, targetEndId, "ROLE"), owningSide,
                relationType(association, sourceEnd, targetEnd), handleMetadata(association, "sourceHandle"),
                handleMetadata(association, "targetHandle"), 1));
        }
        return result;
    }

    private List<DiagramDocument.ClassElement> completeAssociationClassKeys(
        List<Element> packaged, Map<String, Element> elementsById, Map<String, UUID> ids,
        List<DiagramDocument.ClassElement> classes, List<ImportWarning> warnings) {
        Map<UUID, DiagramDocument.ClassElement> byId = new LinkedHashMap<>();
        classes.forEach(value -> byId.put(value.id(), value));
        for (Element associationClass : packaged) {
            if (!"uml:AssociationClass".equals(xmiType(associationClass))) continue;
            List<Element> ends = associationEnds(associationClass, elementsById, elementsById.values());
            if (ends.size() != 2) continue;
            DiagramDocument.ClassElement intermediate = byId.get(ids.get(xmiAttribute(associationClass, "id")));
            if (intermediate == null) continue;
            List<DiagramDocument.Attribute> attributes = new ArrayList<>(intermediate.attributes());
            Set<String> names = new HashSet<>();
            attributes.forEach(value -> names.add(value.name().toLowerCase(Locale.ROOT)));
            for (Element end : ends) {
                DiagramDocument.ClassElement parent = byId.get(ids.get(reference(end, "type")));
                if (parent == null) continue;
                DiagramDocument.Attribute parentKey = referencedKey(parent);
                String keyName = parent.name() + "_" + (parentKey == null ? "id" : parentKey.name());
                if (names.contains(keyName.toLowerCase(Locale.ROOT))) continue;
                String externalId = xmiAttribute(associationClass, "id") + ":key:" + parent.id();
                attributes.add(new DiagramDocument.Attribute(stableId(externalId), keyName,
                    parentKey == null ? "UUID" : parentKey.type(), true, true, false, 1));
                names.add(keyName.toLowerCase(Locale.ROOT));
                warnings.add(warning("ASSOCIATION_CLASS_KEY_DERIVED",
                    "Se calculó la clave foránea y primaria '" + keyName + "' de la clase intermedia",
                    xmiAttribute(associationClass, "id"), "ASSOCIATION_CLASS"));
            }
            byId.put(intermediate.id(), new DiagramDocument.ClassElement(intermediate.id(), intermediate.name(),
                List.copyOf(attributes), intermediate.position(), intermediate.version()));
        }
        return classes.stream().map(value -> byId.get(value.id())).toList();
    }

    private DiagramDocument.Attribute referencedKey(DiagramDocument.ClassElement parent) {
        List<DiagramDocument.Attribute> primaryKeys = parent.attributes().stream()
            .filter(DiagramDocument.Attribute::primaryKey).toList();
        if (primaryKeys.size() == 1) return primaryKeys.getFirst();
        return parent.attributes().stream().filter(value -> value.name().equalsIgnoreCase("id"))
            .findFirst().orElse(parent.attributes().isEmpty() ? null : parent.attributes().getFirst());
    }

    private List<Element> associationEnds(Element association, Map<String, Element> elementsById,
                                          Iterable<Element> allProperties) {
        List<Element> ends = new ArrayList<>();
        for (String memberEnd : references(association, "memberEnd")) {
            Element referenced = elementsById.get(memberEnd);
            if (referenced != null) ends.add(referenced);
    ...
    private String relationType(Element association, Element sourceEnd, Element targetEnd) {
        String metadata = association.getAttributeNS(COLLAB, "relationType");
        if (Set.of("ASSOCIATION", "DEPENDENCY", "AGGREGATION", "COMPOSITION").contains(metadata)) return metadata;
        String aggregation = targetEnd.getAttribute("aggregation");
        if (aggregation.isBlank()) aggregation = sourceEnd.getAttribute("aggregation");
        return switch (aggregation.toLowerCase(Locale.ROOT)) {
    ...
    private String handleMetadata(Element association, String name) {
        String value = association.getAttributeNS(COLLAB, name);
    ...
    private List<DiagramDocument.Generalization> readGeneralizations(List<Element> packaged, Map<String, UUID> ids,
        ...
        for (Element childElement : packaged) {
            if (!isClassType(xmiType(childElement))) continue;
            ...
            for (Element value : directChildren(childElement, "generalization")) {
                ...
                UUID parent = ids.get(reference(value, "general"));
    ...
    private List<DiagramDocument.PackageElement> readPackages(Map<String, Element> packageElements,
        ...
        for (Map.Entry<String, Element> entry : packageElements.entrySet()) {
            ...
            for (Element child : directChildren(element, "packagedElement")) {
                if (Set.of("uml:Class", "uml:AssociationClass", "uml:Enumeration", "uml:Association", "uml:Dependency").contains(xmiType(child))) {
    ...
    private List<Element> modelElements(Document document) {
        LinkedHashMap<String, Element> result = new LinkedHashMap<>();
        for (String elementName : List.of("packagedElement", "ownedMember")) {
            for (Element element : elements(document, elementName)) {
                String id = xmiAttribute(element, "id");
                result.putIfAbsent(id.isBlank() ? elementName + ":" + result.size() : id, element);
            }
        }
        return new ArrayList<>(result.values());
    }

    private Map<String, Element> elementsById(Document document) {
    ...
    private boolean isClassType(String type) {
        return "uml:Class".equals(type) || "uml:AssociationClass".equals(type);
    }

    private boolean isAssociationType(String type) {
        return "uml:Association".equals(type) || "uml:AssociationClass".equals(type);
    }

    private String reference(Element element, String name) {
        List<String> values = references(element, name);
        return values.isEmpty() ? "" : values.getFirst();
    }

    private List<String> references(Element element, String name) {
        List<String> result = new ArrayList<>();
        addReferenceTokens(result, element.getAttribute(name));
        for (Element child : directChildren(element, name)) {
            String value = xmiAttribute(child, "idref");
            if (value.isBlank()) value = child.getAttribute("href");
            if (value.isBlank()) value = child.getAttribute("type");
            if (value.isBlank()) value = child.getAttribute("name");
            addReferenceTokens(result, value);
        }
        return result;
    }

    private void addReferenceTokens(List<String> result, String raw) {
        if (raw == null || raw.isBlank()) return;
        for (String token : raw.trim().split("\\s+")) {
            int fragment = token.lastIndexOf('#');
            String value = fragment >= 0 ? token.substring(fragment + 1) : token;
    ...
    private String bound(Element owner, String childName, String fallback) {
        String directName = "lowerValue".equals(childName) ? "lower" : "upperValue".equals(childName) ? "upper" : "";
        if (!directName.isBlank()) {
            String direct = owner.getAttribute(directName);
            if (!direct.isBlank()) return "-1".equals(direct) ? "*" : direct;
        }
    ...
    private String identifier(String raw, String fallback, List<ImportWarning> warnings,
        ...
        if (Set.of("CLASS", "ENUMERATION").contains(elementType) && value.contains(":")) {
    ...
    private Map<UUID, IntermediateClass> intermediateClasses(DiagramDocument diagram) {
        Map<UUID, DiagramDocument.ClassElement> classes = new HashMap<>();
        diagram.classes().forEach(value -> classes.put(value.id(), value));
        Map<UUID, IntermediateClass> result = new HashMap<>();
        for (DiagramDocument.ClassElement candidate : diagram.classes()) {
            List<DiagramDocument.Association> links = diagram.associations().stream()
                .filter(value -> value.targetId().equals(candidate.id())
                    && "ASSOCIATION".equals(value.relationType())
                    && "1".equals(value.sourceCardinality())
                    && value.targetCardinality().endsWith("*"))
                .toList();
            if (links.size() != 2 || links.get(0).sourceId().equals(links.get(1).sourceId())) continue;
            long primaryKeys = candidate.attributes().stream().filter(DiagramDocument.Attribute::primaryKey).count();
            if (primaryKeys != 2) continue;
            boolean calculatedKeys = links.stream().allMatch(link -> {
                DiagramDocument.ClassElement parent = classes.get(link.sourceId());
                if (parent == null) return false;
                List<DiagramDocument.Attribute> parentKeys = parent.attributes().stream()
                    .filter(DiagramDocument.Attribute::primaryKey).toList();
                if (parentKeys.size() != 1) return false;
                DiagramDocument.Attribute parentKey = parentKeys.getFirst();
                String expected = parent.name() + "_" + parentKey.name();
                String legacy = parent.name() + "_id";
                return candidate.attributes().stream().anyMatch(attribute -> attribute.primaryKey()
                    && attribute.required() && attribute.type().equals(parentKey.type())
                    && (attribute.name().equalsIgnoreCase(expected) || attribute.name().equalsIgnoreCase(legacy)));
            });
            if (calculatedKeys) result.put(candidate.id(), new IntermediateClass(candidate, links.get(0), links.get(1)));
        }
        return result;
    }
    ...
    private record IntermediateClass(DiagramDocument.ClassElement value,
                                     DiagramDocument.Association firstLink,
                                     DiagramDocument.Association secondLink) {}
