...
class GeneratedBackendPostgresIntegrationTest {
    ...
    @Test
    void explicitJoinClassEnforcesBothForeignKeysInPostgres() throws Exception {
        UUID sale = UUID.randomUUID(), product = UUID.randomUUID(), detail = UUID.randomUUID();
        var classes = List.of(
            new DiagramDocument.ClassElement(sale, "Venta", List.of(
                new DiagramDocument.Attribute(UUID.randomUUID(), "id", "UUID", true, true, true)), new DiagramDocument.Position(0, 0), 1),
            new DiagramDocument.ClassElement(product, "Producto", List.of(
                new DiagramDocument.Attribute(UUID.randomUUID(), "id", "UUID", true, true, true)), new DiagramDocument.Position(300, 0), 1),
            new DiagramDocument.ClassElement(detail, "Venta_Producto", List.of(
        ...
        var links = List.of(
            new DiagramDocument.Association(UUID.randomUUID(), sale, detail, "1", "0..*", null, 1),
            new DiagramDocument.Association(UUID.randomUUID(), product, detail, "1", "0..*", null, 1));
        var model = new DiagramDocument(UUID.randomUUID(), "Ventas", 1, classes, links);
        String sql = extractZip(generator.generate(model, "com.example", "ventas-api"))
            .get("src/main/resources/db/migration/V1__initial_schema.sql");
        executeMigrationSql(sql);
        UUID saleKey = UUID.randomUUID(), productKey = UUID.randomUUID();
        jdbcTemplate.update("insert into venta (id) values (?)", saleKey);
        jdbcTemplate.update("insert into producto (id) values (?)", productKey);
        jdbcTemplate.update("insert into venta_producto (venta_id, producto_id) values (?, ?)", saleKey, productKey);
        assertEquals(1, jdbcTemplate.queryForObject("select count(*) from venta_producto", Integer.class));
        assertThrows(DataAccessException.class, () -> jdbcTemplate.update(
            "insert into venta_producto (venta_id, producto_id) values (?, ?)", UUID.randomUUID(), productKey));
    }
