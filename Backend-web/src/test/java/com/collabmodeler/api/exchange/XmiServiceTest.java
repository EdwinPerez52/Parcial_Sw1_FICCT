...
class XmiServiceTest {
    ...
    @Test
    void importsQualifiedEaClassNamesAsSimpleTableNames() {
        String xml = """
            <xmi:XMI xmlns:xmi="http://schema.omg.org/spec/XMI/2.1" xmi:version="2.1">
              <uml:Model xmlns:uml="http://schema.omg.org/spec/UML/2.1" xmi:type="uml:Model" xmi:id="model" name="VideoClub">
                <packagedElement xmi:type="uml:Class" xmi:id="actor" name="VideoClub_AlquilerPeliculas::Actor"/>
                <packagedElement xmi:type="uml:Class" xmi:id="pelicula" name="VideoClub_AlquilerPeliculas:pelicula"/>
              </uml:Model>
            </xmi:XMI>
            """;
        var imported = service.importXmi(xml.getBytes(StandardCharsets.UTF_8), "archivo");
        assertEquals(Set.of("Actor", "pelicula"), imported.diagram().classes().stream()
            .map(DiagramDocument.ClassElement::name).collect(Collectors.toSet()));
        String exported = service.exportXmi(imported.diagram());
        assertTrue(exported.contains("name=\"Actor\""));
        assertTrue(exported.contains("name=\"pelicula\""));
        assertFalse(exported.contains("VideoClub_AlquilerPeliculas:"));
    }

    @Test
    void roundTripsDependencyAndCompositionTypes() {
        UUID a = UUID.randomUUID(), b = UUID.randomUUID();
        var classes = List.of(
            new DiagramDocument.ClassElement(a, "Origen", List.of(), new DiagramDocument.Position(0, 0), 1),
            new DiagramDocument.ClassElement(b, "Destino", List.of(), new DiagramDocument.Position(200, 0), 1));
        var links = List.of(
            new DiagramDocument.Association(UUID.randomUUID(), a, b, "1", "1", "usa", null, null,
                "SOURCE", "DEPENDENCY", "bottom-50", "top-50", 1),
            new DiagramDocument.Association(UUID.randomUUID(), a, b, "1", "0..*", "contiene", null, null,
                "SOURCE", "COMPOSITION", "right-75", "left-25", 1));
        var model = new DiagramDocument(UUID.randomUUID(), "Relaciones", 1, classes, links);
        var imported = service.importXmi(service.exportXmi(model).getBytes(StandardCharsets.UTF_8), "fallback");
        assertEquals(Set.of("DEPENDENCY", "COMPOSITION"), imported.diagram().associations().stream()
            .map(DiagramDocument.Association::relationType).collect(Collectors.toSet()));
        assertEquals(Set.of("bottom-50", "right-75"), imported.diagram().associations().stream()
            .map(DiagramDocument.Association::sourceHandle).collect(Collectors.toSet()));
    }
    ...
    @Test
    void exportsAndReimportsIntermediateClassAsAssociationClass() {
        ...
        var sale = new DiagramDocument.ClassElement(saleId, "Venta", List.of(
            new DiagramDocument.Attribute(UUID.randomUUID(), "numero", "Long", true, true, true)),
            new DiagramDocument.Position(0, 0), 1);
        var product = new DiagramDocument.ClassElement(productId, "Producto", List.of(
            new DiagramDocument.Attribute(UUID.randomUUID(), "codigo", "UUID", true, true, true)),
            new DiagramDocument.Position(300, 0), 1);
        var detail = new DiagramDocument.ClassElement(detailId, "Detalle", List.of(
            new DiagramDocument.Attribute(UUID.randomUUID(), "Venta_numero", "Long", true, true, false),
            new DiagramDocument.Attribute(UUID.randomUUID(), "Producto_codigo", "UUID", true, true, false),
            new DiagramDocument.Attribute(UUID.randomUUID(), "cantidad", "Integer", false, true, false)),
            new DiagramDocument.Position(150, 200), 1);
        var saleLink = new DiagramDocument.Association(UUID.randomUUID(), saleId, detailId,
            "1", "0..*", null, null, null, "TARGET", 1);
        var productLink = new DiagramDocument.Association(UUID.randomUUID(), productId, detailId,
            "1", "1..*", null, null, null, "TARGET", 1);
        var model = new DiagramDocument(UUID.randomUUID(), "Ventas", 1,
            List.of(sale, product, detail), List.of(saleLink, productLink));

        String xml = service.exportXmi(model);
        var imported = service.importXmi(xml.getBytes(StandardCharsets.UTF_8), "fallback").diagram();

        assertTrue(xml.contains("xmi:type=\"uml:AssociationClass\""));
        assertEquals(2, imported.associations().stream().filter(value -> value.targetId().equals(
            imported.classes().stream().filter(item -> item.name().equals("Detalle")).findFirst().orElseThrow().id())).count());
        var importedDetail = imported.classes().stream().filter(value -> value.name().equals("Detalle")).findFirst().orElseThrow();
        assertEquals(List.of("Venta_numero", "Producto_codigo"), importedDetail.attributes().stream()
            .filter(DiagramDocument.Attribute::primaryKey).map(DiagramDocument.Attribute::name).toList());
        assertEquals(Set.of("0..*", "1..*"), imported.associations().stream()
            .filter(value -> value.targetId().equals(importedDetail.id()))
            .map(DiagramDocument.Association::targetCardinality).collect(Collectors.toSet()));
    }
    ...
    @Test
    void importsReferencedEndsDirectBoundsAndAssociationClasses() {
        String xml = """
            <?xml version="1.0" encoding="UTF-8"?>
            <xmi:XMI xmi:version="2.1" xmlns:xmi="http://schema.omg.org/spec/XMI/2.1" xmlns:uml="http://schema.omg.org/spec/UML/2.1">
              <uml:Model xmi:type="uml:Model" xmi:id="model" name="Ventas">
                <packagedElement xmi:type="uml:Package" xmi:id="domain" name="Dominio">
                  <packagedElement xmi:type="uml:Class" xmi:id="customer" name="cliente">
                    <ownedAttribute xmi:type="uml:Property" xmi:id="customer-id" name="ci"/>
                    <ownedAttribute xmi:type="uml:Property" xmi:id="customer-sale-end" name="venta" association="sale-customer" lower="1" upper="1">
                      <type xmi:idref="customer"/>
                    </ownedAttribute>
                  </packagedElement>
                  <packagedElement xmi:type="uml:Class" xmi:id="product" name="producto">
                    <ownedAttribute xmi:type="uml:Property" xmi:id="product-id" name="id"/>
                  </packagedElement>
                  <packagedElement xmi:type="uml:Class" xmi:id="seller" name="vendedor"/>
                  <packagedElement xmi:type="uml:Class" xmi:id="sale" name="venta">
                    <ownedAttribute xmi:type="uml:Property" xmi:id="sale-id" name="id"/>
                    <ownedAttribute xmi:type="uml:Property" xmi:id="sale-customer-end" name="ventas" association="sale-customer" lower="0" upper="*">
                      <type xmi:idref="sale"/>
                    </ownedAttribute>
                  </packagedElement>
                  <ownedMember xmi:type="uml:Association" xmi:id="sale-customer" name="compra">
                    <memberEnd xmi:idref="sale-customer-end"/>
                    <memberEnd xmi:idref="customer-sale-end"/>
                  </ownedMember>
                  <ownedMember xmi:type="uml:Association" xmi:id="sale-seller" memberEnd="sale-seller-end seller-sale-end">
                    <ownedEnd xmi:type="uml:Property" xmi:id="sale-seller-end">
                      <type href="model.xmi#sale"/>
                      <lowerValue value="0"/><upperValue value="*"/>
                    </ownedEnd>
                    <ownedEnd xmi:type="uml:Property" xmi:id="seller-sale-end">
                      <type xmi:idref="seller"/>
                      <lowerValue value="1"/><upperValue value="1"/>
                    </ownedEnd>
                  </ownedMember>
                  <packagedElement xmi:type="uml:AssociationClass" xmi:id="detail" name="detalle" memberEnd="detail-sale detail-product">
                    <ownedAttribute xmi:type="uml:Property" xmi:id="quantity" name="cantidad"/>
                    <ownedAttribute xmi:type="uml:Property" xmi:id="unit-price" name="precioUnit"/>
                    <ownedEnd xmi:type="uml:Property" xmi:id="detail-sale" lower="0" upper="*">
                      <type xmi:idref="sale"/>
                    </ownedEnd>
                    <ownedEnd xmi:type="uml:Property" xmi:id="detail-product" lower="1" upper="*">
                      <type xmi:idref="product"/>
                    </ownedEnd>
                  </packagedElement>
                </packagedElement>
              </uml:Model>
        ...
        assertEquals(Set.of("cliente", "producto", "vendedor", "venta", "detalle"), imported.diagram().classes().stream()
            .map(DiagramDocument.ClassElement::name).collect(Collectors.toSet()));
        assertEquals(4, imported.diagram().associations().size());
        assertTrue(imported.diagram().associations().stream().anyMatch(value ->
            value.name() != null && value.name().equals("compra")
                && value.sourceCardinality().equals("0..*") && value.targetCardinality().equals("1")));
        var detail = imported.diagram().classes().stream()
            .filter(value -> value.name().equals("detalle")).findFirst().orElseThrow();
        assertEquals(2, imported.diagram().associations().stream()
            .filter(value -> value.targetId().equals(detail.id())).count());
        assertEquals(List.of("cantidad", "precioUnit", "venta_id", "producto_id"), detail.attributes().stream()
            .map(DiagramDocument.Attribute::name).toList());
        assertEquals(2, detail.attributes().stream().filter(DiagramDocument.Attribute::primaryKey).count());
        assertTrue(imported.warnings().stream().noneMatch(value -> value.code().equals("UNSUPPORTED_PROPERTY_TYPE")));
    }
